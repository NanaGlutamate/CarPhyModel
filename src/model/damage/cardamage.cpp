...
void DamageSystem::tick(double dt, Components& c){
    ...
    for(auto&& fireEvent : c.getSpecificSingletonComponent<FireEventQueue>().value()){
        ...
        for(auto&& [id, _damage_model, _coordinate] : c.getNormalComponents<PartDamageModel, Coordinate>()){
