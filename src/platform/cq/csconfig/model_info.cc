...
// 获得结构体数据
const ModelInfo &CSModelInfo::GetInfo() { return info; }
...
bool CSModelInfo::ReadFile(const std::string &config_filepath) {
    ...
    if (!ifs)
        return false;
    std::string config_content = std::string(
...
bool CSModelInfo::WriteXmlDoc(xml_document<char> &doc) {
    ...
    root->append_attribute(
        doc.allocate_attribute("id", doc.allocate_string(info.id.c_str())));
    root->append_attribute(
        doc.allocate_attribute("name", doc.allocate_string(info.name.c_str())));
    root->append_attribute(doc.allocate_attribute(
        "displayName", doc.allocate_string(info.display_name.c_str())));
    root->append_attribute(doc.allocate_attribute(
        "dllName", doc.allocate_string(info.dll_name.c_str())));
    root->append_attribute(doc.allocate_attribute(
        "hasGeoInfo",
        doc.allocate_string(info.has_geo_info ? "true" : "false")));
    root->append_attribute(doc.allocate_attribute(
        "category", doc.allocate_string(info.category.c_str())));
    root->append_attribute(doc.allocate_attribute(
        "multiples", doc.allocate_string(info.multiples.c_str())));
    ...
    for (const auto &[_, params] : info.params) {
        ...
        model_parameter->append_attribute(doc.allocate_attribute(
            "name", doc.allocate_string(params.name.c_str())));
        model_parameter->append_attribute(doc.allocate_attribute(
            "type", doc.allocate_string(params.type.c_str())));
        model_parameter->append_attribute(doc.allocate_attribute(
            "displayName", doc.allocate_string(params.display_name.c_str())));
        model_parameter->append_attribute(doc.allocate_attribute(
            "usage", doc.allocate_string(params.usage.c_str())));
        model_parameter->append_attribute(doc.allocate_attribute(
            "value", doc.allocate_string(params.value.c_str())));
        model_parameter->append_attribute(doc.allocate_attribute(
            "unit", doc.allocate_string(params.unit.c_str())));
