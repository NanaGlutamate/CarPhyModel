...
// 获得TypeDefineInfo
const TypeDefineInfo & CSTypeDefineInfo::GetInfo() {
  return info;
}
...
bool CSTypeDefineInfo::ReadContent(const std::string &content) {
  ...
  try {
    ...
    for (auto it = type_node->first_node("Type"); it;
      ...
      // 获取类型具体信息
      for (auto it_p = param_node->first_node(); it_p; it_p = it_p->next_sibling()) {
...
bool CSTypeDefineInfo::WriteXmlDoc(xml_document<char> &doc) {
  ...
  xml_node<> *Type_definition = doc.allocate_node(node_element,
                                "Types");
  ...
  for (const auto&[_, type_define] : info.type_defines) {
    ...
    type_node->append_attribute(doc.allocate_attribute("name",
                                type_define.name.c_str()));
    ...
    for (const auto&[param_name, param_type] : type_define.params) {
      ...
      param->append_attribute(doc.allocate_attribute("name",
                              param_name.c_str()));
      param->append_attribute(doc.allocate_attribute("type",
                              param_type.c_str()));
      auto unit = type_define.param_unit.find(param_name);
      if(unit != type_define.param_unit.end())
        param->append_attribute(doc.allocate_attribute("unit",
                                unit->second.c_str()));
