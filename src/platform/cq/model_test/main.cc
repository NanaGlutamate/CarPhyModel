...
// time_point转字符串
std::string TimePointToStringMilliSeconds(std::chrono::time_point<std::chrono::system_clock>);

// 获得time_point数据结构
std::chrono::system_clock::time_point GetTimePoint(uint16_t, uint16_t, uint16_t, uint16_t,
                                                   uint16_t, uint16_t);

// 获得当前时间字符串
std::string GetCurrentTimeStr(const std::chrono::system_clock::time_point &, uint64_t);
...
// 输出参数类型校验及数值打印
bool PrintOutputParams(std::unordered_map<std::string, std::any> *, const ModelInfo &,
                       const TypeDefineInfo &);
...
int main(int argc, char *argv[]) {
    ...
    // 日志输出回调
    std::function<void(const std::string &, uint32_t)> log_cb_ = [](const std::string &msg_,
                                                                    uint32_t level) {
    ...
    std::function<std::string(const std::string &,
        ...
        com_cb_ =
            [&sim_step, &current_ticks, &sim_current_duration, &sim_start_time](
                ...
                const std::unordered_map<std::string, std::any> &value_) -> std::string {
    ...
    while (1) {
        ...
        std::cout << "当前帧数:" << current_ticks << "\t"
                  ...
                  << "当前仿真时间" << GetCurrentTimeStr(sim_start_time, sim_current_duration)
                  << std::endl
                  << std::endl;
    }
...
// 获取模型的输出参数
bool GetOutputParamsFromModel(CSModelObject *model_obj_, const ModelInfo &model_info_,
    ...
    try {
        ...
        // 获取模型的输出参数
        std::unordered_map<std::string, std::any> *output_params_ = model_obj_->GetOutput();
...
std::string
TimePointToStringMilliSeconds(std::chrono::time_point<std::chrono::system_clock> time_point) {
...
std::chrono::system_clock::time_point GetTimePoint(uint16_t year, uint16_t mon, uint16_t day,
...
std::string GetCurrentTimeStr(const std::chrono::system_clock::time_point &start_time_,
                              uint64_t time_duration) {
    std::chrono::system_clock::time_point current_time =
        start_time_ + std::chrono::milliseconds((long long)time_duration);
...
//
bool PrintOutputParams(std::unordered_map<std::string, std::any> *output_params_,
    ...
    try {
        for (auto &[param_name_, param_info_] : model_info_.params) {
            ...
            if (auto it = output_params_->find(param_name_); it != output_params_->end()) {
        ...
        std::cerr << "参数" << param_name_type_
                  << ") 实际输出数据的类型与模型描述文件中的定义不一致: " << e.what()
                  << std::endl;
        return false;
    } catch (const std::exception &e) {
