...
class CSParamBase {
  public:
    ...
    virtual void FromValueMap(const CSValueMap &) = 0;
...
class CSVector2D final : public CSParamBase {
  public:
    ...
    CSVector2D(const CSValueMap &v) { FromValueMap(v); }

    virtual CSValueMap ToValueMap() override {
        return std::move(CSValueMap{{"x", x}, {"y", y}});
    }
    virtual void FromValueMap(const CSValueMap &v) override {
        for (auto &it : v) {
            if (it.first == "x")
...
class CSVector3D final : public CSParamBase {
  public:
    ...
    CSVector3D(double x_, double y_, double z_) : x(x_), y(y_), z(z_) {}
    CSVector3D(const CSValueMap &v) { FromValueMap(v); }

    virtual CSValueMap ToValueMap() override {
        return std::move(CSValueMap{{"x", x}, {"y", y}, {"z", z}});
    }
    virtual void FromValueMap(const CSValueMap &v) override {
        for (auto &it : v) {
            if (it.first == "x")
...
class CSVector4D final : public CSParamBase {
  public:
    ...
    CSVector4D(double x_, double y_, double z_, double w_)
        : x(x_), y(y_), z(z_), w(w_) {}
    CSVector4D(const CSValueMap &v) { FromValueMap(v); }

    virtual CSValueMap ToValueMap() override {
        return std::move(CSValueMap{{"x", x}, {"y", y}, {"z", z}, {"w", w}});
    }
    virtual void FromValueMap(const CSValueMap &v) override {
        for (auto &it : v) {
            if (it.first == "x")
...
class CSMapPoint final : public CSParamBase {
  public:
    CSMapPoint() = default;
    CSMapPoint(uint64_t id_, const CSVector4D &p) : id(id_), point(p) {}
    CSMapPoint(const CSValueMap &v) { FromValueMap(v); }

    virtual CSValueMap ToValueMap() override {
        return std::move(CSValueMap{{"id", id}, {"point", point.ToValueMap()}});
    }
    virtual void FromValueMap(const CSValueMap &v) override {
        for (auto &it : v) {
            if (it.first == "id")
    ...
    const CSVector4D &GetPoint() const { return point; }
...
class CSMapPath final : public CSParamBase {
  public:
    ...
    CSMapPath(const CSValueMap &v) { FromValueMap(v); }
    ...
    uint64_t GetId() const { return id; }
    const std::vector<CSVector4D> &GetPoints() const { return points; }
...
class CSMapRegion final : public CSParamBase {
  public:
    ...
    CSMapRegion(uint64_t id_, const std::vector<CSVector4D> &points_)
        : id(id_), points(points_) {}
    CSMapRegion(const CSValueMap &v) { FromValueMap(v); }
