...
class CSModelObject {
 ...
 public:
  ...
  // 设置模型实例状态
  void SetState(CSInstanceState state) {
    state_ = state;
  }
  // 获取模型实例状态
  CSInstanceState GetState(void) {
    return state_;
  }
  ...
  // 设置模型实例所属的阵营的唯一标识
  void SetForceSideID(uint16_t force_side_id) {
    force_side_id_ = force_side_id;
  }
  // 获取模型实例所属的阵营的唯一标识
  uint16_t GetForceSideID() const {
    return force_side_id_;
  }
 ...
 protected:
  ...
  // 输出业务消息
  void WriteKeyMessage(const std::vector<std::any> &msgs) {
    params_.emplace("KeyMessages", msgs);
  }

  // 模型实例直接对外写主题 (待调整)
  void WriteTopic(const std::string& topic_name,
                  const std::unordered_map<std::string, std::any> &topic_data) {
  ...
  // 动态创建实体
  void CreateEntity(const std::string& model_id,
                    const std::string& instance_name,
                    uint64_t instance_id,
                    uint16_t forceside_id,
                    std::unordered_map<std::string, std::any>& params) {
    std::unordered_map<std::string, std::any> basic_params = {
      { "InstanceName", instance_name },
      { "ID", instance_id },
      { "ModelID", model_id },
      { "ForceSideID", forceside_id }
    };
 ...
 protected:
  // 通用回调函数
  std::string CommonCallBack(const std::string &fun_name,
 ...
 public:
  // 设置模型日志输出函数
  void SetLogFun(std::function<void(const std::string&, uint32_t)> log) {
    log_ = log;
  }
  // 设置通用回调函数
  void SetCommonCallBack(
    std::function<std::string(const std::string&, const std::unordered_map<std::string, std::any>&)>
    com_cb) {
    com_cb_ = com_cb;
  }
